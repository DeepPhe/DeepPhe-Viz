...
function Patient() {
  ...
  useEffect(() => {
    ...
    const getSummaryPromise = GetSummary(patientId);
    const getPatientFromJsonFilePromise = getNewPatientJsonFromFile(patientId);
    Promise.all([getSummaryPromise, getPatientFromJsonFilePromise]).then(
      (results) => {
        //..prob should set summary here too?
        setPatientJson(results[1]);
      }
    );
  }, [patientId]);
                        ...
                        <GridItem xs={4} sm={4} md={3} id="mentions_container2">
                          ...
                          <GridItem xs={12} id="mentions_container">
                            <GridContainer>
                              ...
                              <GridItem xs={12} id="search_label">
                                Filter Mentions:
                              </GridItem>
                              <GridItem xs={12}>
                                <input
