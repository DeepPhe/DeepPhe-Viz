    ...
    show = (svgContainerId) => {
        ...
        if (d3.select(".biomarkers_chart_group").empty()) {
            ...
            biomarkerStatusGrp.selectAll(".biomarker_status_bar")
                ...
                biomarkerStatusGrp.selectAll(".biomarker_status_bar").classed("selected_biomarker_bar", false)
                ...
                d3.selectAll("#" + d.data.biomarker + "_" + d.status).classed("selected_biomarker_text_color", true)
                ...
                let selectedPatients = that.state.biomarkerData.biomarkerStatsPerPatient.filter((f) => {
                    ...
                    f.relationValueArray.forEach(a => {
                        if (a[0] === d.data.biomarker && a[1] === d.status)
                            result = true;
                    })
                    return result
                })
                ...
                for (let i = stagesInfoCopy.length - 1; i >= 0; i--) {
                    ...
                    for (let j = stagesInfo.patients.length - 1; j >= 0; j--) {
                        ...
                        if (!selectedPatients.map(p => p.patientId).includes(stagesInfo.patients[j].patientId)) {
